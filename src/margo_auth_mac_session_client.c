...
typedef struct {
    ...
    connection_t      connection; // in practice: hash mapping server address to connection_t
} client_t;
...
static int client_authenticate(client_t* client, hg_addr_t address)
{
    ...
    client->connection.session_id = out.session_id;
    client->connection.seq_no     = 0;
...
static int client_hello(client_t* client, hg_addr_t address, const char* name)
{
    ...
    create_token(&in.token,
                 client->connection.session_id,
                 client->connection.seq_no,
                 (const char*)client->key,
                 sizeof(client->key));
    in.name = (char*)name;
    ...
    ret = out.ret;
    if(ret == 0) client->connection.seq_no++;
